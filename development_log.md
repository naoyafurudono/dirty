...
### Phase 2: 基本的なクロスパッケージ解析の実装 ✅

1. **実装内容**
   - `analyzer/crosspackage.go`: クロスパッケージ解析の拡張機能
   - インポート情報の解析とパッケージパスの解決
   - JSON効果定義での完全修飾名サポート（後方互換性あり）
   - 合成関数情報の生成による外部パッケージ関数の追跡

2. **技術的詳細**
   - ASTベースの解析を維持（go/types未使用）
   - `EnhanceWithCrossPackageSupport`関数で既存解析を拡張
   - インポートエイリアスの解決
   - 完全修飾名（例：`github.com/example/pkg1.GetUser`）での関数識別

3. **テストケース**
   - `testdata/src/crosspackage/`: 3つのパッケージ間での効果伝播テスト
   - `example/crosspackage/`: 実用的な例（db → service → handler）

4. **現在の制限事項**
   - analysistestフレームワークでのマルチパッケージテストに課題
   - メソッドレシーバーの型情報が不完全
   - インターフェース経由の呼び出しは未対応

5. **動作確認**
   - 単一パッケージ内でのクロスパッケージ呼び出しは正しく解析
   - JSON効果定義による外部関数の効果指定が機能
   - 効果の伝播と整合性チェックが動作

### 次のステップ
- analysistestの制限を回避する統合テスト方法の検討
- Phase 1（go/types統合）による型情報の正確な解決
- JSON Effects V2の実装

## 2025-07-21: クロスパッケージ解析のFactベース実装

### 背景
Phase 2のクロスパッケージ解析実装が完了したが、テストで期待通りに動作しないことが判明。

### 調査結果
1. **analysisパッケージの処理順序**
   - パッケージは依存関係に基づいてトポロジカルソートされる
   - インポートされるパッケージが先に解析される
   - これによりFactが正しく伝播する

2. **問題の原因**
   - Fact自体は正しく伝播している（デバッグ出力で確認）
   - CallSiteの追加処理に問題があった
   - 効果の伝播処理が不完全
...
#### 1. Factメカニズムの実装
```go
// PackageEffectsFact: パッケージ内の全関数の効果情報を保持
type PackageEffectsFact struct {
    FunctionEffects map[string][]string
}

// FunctionEffectsFact: 個別の関数の効果情報を保持
...
#### 2. デバッグ機能の追加
- `DIRTY_VERBOSE=1`でFactの伝播状況を確認可能
- 各フェーズでの詳細な情報出力

#### 3. multicheckerコマンドの追加
- `cmd/dirty-multi`: 複数パッケージの解析に対応

### 現在の状態
- Factの基本的な伝播は動作している
- クロスパッケージ呼び出しの検出も動作
- エラー報告が期待通りに動作しない問題が残っている
...
### 技術的な判断
- 当初はFactメカニズム以外の実装方法も検討
- go/packagesを直接使う方法、ハイブリッドアプローチなど
- 最終的にFactメカニズムが最適と判断（analysisパッケージの設計に合致）

### 学んだこと
- analysisパッケージはトポロジカルソートでパッケージを処理
- Factメカニズムは標準的な方法でパッケージ間情報を共有
- analysistestフレームワークはFactをサポートしない（"unexpected fact"として扱われる）
...
1. **現状のFacts使用における問題点**
   - Factsの有無が診断メッセージに影響している
   - `foundInFacts`フラグが動作を変更してしまっている
   - テストがFactsの有無に依存している

2. **printf analyzerのパターン研究**
   - Factsは純粋に情報共有のみに使用
   - 診断は常にローカルな解析に基づく
   - Factsはwrapper関数の識別にのみ使用、診断生成には影響しない

3. **新アーキテクチャの設計**
   - **設計原則**：
     - Factsは任意の最適化
     - 診断はローカル解析から生成
     - 情報源の分離（Facts/JSON/ローカル）
   
   - **実装設計**：
     - `EffectResolver`: 統一的な効果解決層
     - 情報源に関わらず同じ診断を生成
     - Factsの有無でテストが変わらない

4. **実装計画**
   - Phase 1: 効果解決のリファクタリング（低リスク）
   - Phase 2: 診断の分離（中リスク）
   - Phase 3: アーキテクチャの簡素化（高リスク）
   - Phase 4: 最適化（機能強化）

詳細設計: `docs/facts-redesign.md`
